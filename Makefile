...
# Per-mon menu icons: full-color PNGs; rgbgfx maps them to 4-color tiles (--colors auto).
gfx/icons/%.2bpp: gfx/icons/%.png
...
# Pokémon PNGs often have anti-aliased edges (many near-gray values). `--colors dmg` requires
# strict R=G=B grays and rejects those; `--colors auto` quantizes to 4 colors for 2bpp (like gfx/icons).
gfx/pokemon/%.2bpp: gfx/pokemon/%.png
	$(RGBGFX) --colors auto $(RGBGFXFLAGS) -o $@ $<
	$(if $(tools/gfx),\
		tools/gfx $(tools/gfx) -o $@ $@ || $$($(RM) $@ && false))
...
# Extended-species fronts/backs: same as all other mons — place gfx/pokemon/front/<name>.png
# (and backs *b.png); the %.2bpp: %.png rule builds tiles, then %.pic compresses them.
