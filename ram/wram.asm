...
; power of the enemy's last-selected move (used by trainer move AI anti-spam heuristics)
wAILastMovePower:: db
...
; If non-zero, the next SelectEnemyMove forces CANNOT_MOVE ($ff) for the enemy.
...
; Non-zero after the player's first SendOutMon completes its throw palette refresh.
; Used so EnemySendOutFirstMon forces hero palette during opening opponent send-out
; even if wBattleMonSpecies is stale/non-zero (Red-only palette glitch otherwise).
wPlayerBattleSendOutComplete:: db
...
; 1 when TryFieldHMInteraction / boulder strength handled the A press
wFieldHMInteractionResult:: db
; Non-zero: UsedCut skips GBPalWhiteOutWithDelay3 (field A-button HM only)
wFieldHMCutSkipPalWhiteout:: db
