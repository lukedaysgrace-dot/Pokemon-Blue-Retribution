...
.gymleader
	...
	jr z, .rivalDefeatMusic
	cp BLUE_CLOAK
	jr z, .blueCloakDefeatMusic
	jr .notrival
.blueCloakDefeatMusic
; Same victory sting as the final rival, but do not set BIT_NO_MAP_MUSIC:
; that bit makes LoadMapData skip PlayDefaultMusicFadeOutCurrent, so
; overworld would keep battle audio until changing maps (RIVAL3 relies on
; scripts such as Hall of Fame / Oak's lab to clear the flag).
	ld b, MUSIC_DEFEATED_GYM_LEADER
	jr .notrival
.rivalDefeatMusic
	ld b, MUSIC_DEFEATED_GYM_LEADER
...
SwitchPlayerMon:
; End opponent multi-turn trapping (Wrap, Fire Spin, Clamp, etc.) when the player switches.
; Also reset enemy move selection so SelectEnemyMove runs fresh next turn (see SelectEnemyMove early ret).
	ld hl, wPlayerBattleStatus1
	res USING_TRAPPING_MOVE, [hl]
	ld a, [wEnemyBattleStatus1]
	bit USING_TRAPPING_MOVE, a
	jr z, .enemyNotTrapping
	ld hl, wEnemyBattleStatus1
	res USING_TRAPPING_MOVE, [hl]
	xor a
...
; Next SelectEnemyMove must not overwrite with a new move (see SelectEnemyMove). Link battles keep vanilla exchange.
	ld a, [wLinkState]
	cp LINK_STATE_BATTLING
	jr z, .enemyNotTrapping
	ld a, 1
	ld [wEnemySkipNextMoveSelectionAfterTrapSwitch], a
.enemyNotTrapping
	callfar RetreatMon
	ld c, 50
	call DelayFrames
	...
	predef FlagActionPredef
	call LoadBattleMonFromParty
; New Pokémon must not inherit volatile battle state from the outgoing one.
; (LoadBattleMonFromParty does not clear these; stale THRASHING_ABOUT / CHARGING_UP /
; STORING_ENERGY etc. can make MainInBattleLoop skip MoveSelectionMenu while still
; running SelectEnemyMove, giving the enemy an extra action before the player picks a move.)
	xor a
	...
	ld a, [hl]
	and a
	jr nz, .storePlayerSelectedMove
	ld a, STRUGGLE
.storePlayerSelectedMove
	ld [wPlayerSelectedMove], a
	call SendOutMon
	call SaveScreenTilesToBuffer1
	ld a, $2
...
.noLinkBattle
	...
	ret nz
	ld a, [wEnemySkipNextMoveSelectionAfterTrapSwitch]
	and a ; player switched out of our trap: waste this pick (shinpokered / Stadium behavior)
	jr z, .noTrapSwitchSkip
	xor a
	ld [wEnemySkipNextMoveSelectionAfterTrapSwitch], a
	jr .unableToSelectMove
.noTrapSwitchSkip
	ld a, [wPlayerBattleStatus1]
...
; FLASH, HAZE*, SCREECH, LIGHT SCREEN*, REFLECT*
...
; subtracts the three-byte number at hl from the one at bc and stores it at de
SubThreeByteNum:
	call .subByte
	call .subByte
.subByte
...
InitWildEnemyPartyFromLoadedMon::
; wEnemyPartyCount shares WRAM with wGrassRate; wild battles never ran ReadTrainer, so the
; "party count" was still the grass encounter rate and wEnemyPartySpecies held level/species
; slot bytes. Build a clean 1-mon party so DrawAllPokeballs / send-out logic never reads junk.
	ld hl, wEnemyPartyCount
	xor a
	ld [hli], a
	ld b, PARTY_LENGTH + 1
...
InitWildBattle:
	...
	ld [wIsInBattle], a
	call LoadEnemyMonData
	call InitWildEnemyPartyFromLoadedMon
	call DoBattleTransitionAndInitBattleVariables
	ld a, [wCurOpponent]
	cp RESTLESS_SOUL
	jr z, .isGhost
