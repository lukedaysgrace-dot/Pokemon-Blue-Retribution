...
.outOfBattleMovePointers
	...
	dw .softboiled
.fly
	farcall VerifyFlyAllowFromPartyMenu
	jp nc, .loop
...
.canFly
	...
	jp StartMenu_Pokemon
.cut
	farcall VerifyCutAllowFromPartyMenu
	jp nc, .loop
	predef UsedCut
	...
	jp CloseTextDisplay
.surf
	farcall VerifySurfAllowFromPartyMenu
	jp nc, .loop
	...
	jp .goBackToMap
.strength
	farcall VerifyStrengthAllowFromPartyMenu
	jp nc, .loop
	predef PrintStrengthText
	call GBPalWhiteOutWithDelay3
	jp .goBackToMap
.flash
	farcall VerifyFlashAllowFromPartyMenu
	jp nc, .loop
	xor a
	ld [wMapPalOffset], a
