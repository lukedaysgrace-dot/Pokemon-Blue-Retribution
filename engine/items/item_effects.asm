...
ItemUseTownMap:
	...
	farcall LoadTownMap_Fly
	jr ItemUseTownMap_done
.townMapViewOnly
	farcall DisplayTownMap
ItemUseTownMap_done:
; Close item/start menu after map (same idea as party FLY .goBackToMap).
	ld a, [wStatusFlags6]
	bit BIT_FLY_WARP, a
	jr z, .townMapSkipFlyCleanup
...
; try to Surf
	...
	jp c, SurfingAttemptFailed
; Text before surf state: PrintText runs UpdateSprites after drawing the box.
...
.stopSurfing
	...
	ld [wJoyIgnore], a
	call PlayDefaultMusic ; play walking music
	jp _LoadWalkingPlayerSpriteGraphics
