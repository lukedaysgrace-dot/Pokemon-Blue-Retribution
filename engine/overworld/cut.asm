...
.canCut
	...
	ld a, [wFieldHMCutSkipPalWhiteout]
	and a
	jr z, .doCutPalWhiteout
	xor a
	ld [wFieldHMCutSkipPalWhiteout], a
	jr .afterCutPalWhiteout
.doCutPalWhiteout
	call GBPalWhiteOutWithDelay3
.afterCutPalWhiteout
; Hide the window and stop BG DMA before Restore: LoadScreenTilesFromBuffer2
; replaces the whole tilemap with the saved overworld while InitMapSprites (LCD
; off) reshuffles VRAM — if the window is on, that frame shows map indices in
; the text area with wrong pattern data (sprite junk). Same for hAutoBGTransfer.
